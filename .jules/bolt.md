...
## 2025-02-13 - Avoid Intermediate Array Allocation in Image Masking
**Learning:** In image processing pipelines (e.g., `preprocess.py`), using functions like `cv2.bitwise_and(image, image, mask=mask)` to extract features from specific pixels needlessly allocates a full-size intermediate image array with a black background, simply to compute statistical means over the masked area.
**Action:** Always prefer direct NumPy boolean indexing (e.g., `image[:, :, 0][mask > 0]`) when extracting pixel values for calculations. This targets only the required data, bypassing the memory allocation and CPU cycles needed to construct and process a redundant full-size array.
