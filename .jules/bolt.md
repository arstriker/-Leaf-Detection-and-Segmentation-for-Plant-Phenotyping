...
## 2024-05-18 - Replacing bitwise_and with boolean indexing
**Learning:** Using `cv2.bitwise_and` followed by boolean mask indexing allocates a large, unnecessary full-size intermediate NumPy array, causing a measurable spike in memory overhead (e.g., from ~38MB down to ~9MB for a 4K image).
**Action:** When extracting subset pixel values from an image based on a mask, directly apply the boolean mask to the original array (e.g., `image[:,:,0][valid_mask]`) to bypass intermediate full-array allocations.
