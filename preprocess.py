...
def extract_features(image, mask, gray_image=None, lbp=None):
    ...
    # Performance optimization: Avoid allocating a full-size intermediate array with cv2.bitwise_and
    # by directly applying boolean indexing on the original image channels to extract masked pixels.
    R = image[:,:,0][mask > 0]
    G = image[:,:,1][mask > 0]
    B = image[:,:,2][mask > 0]
