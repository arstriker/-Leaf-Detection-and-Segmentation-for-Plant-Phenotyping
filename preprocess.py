...
def extract_features(image, mask, gray_image=None, lbp=None):
    ...
    # Performance optimization: Use direct boolean indexing instead of allocating an intermediate image
    # with cv2.bitwise_and. This avoids unnecessary memory allocation and CPU overhead.
    mask_bool = mask > 0
    R = image[:,:,0][mask_bool]
    G = image[:,:,1][mask_bool]
    B = image[:,:,2][mask_bool]
