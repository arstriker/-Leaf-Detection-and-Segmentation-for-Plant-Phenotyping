...
def extract_features(image, mask, gray_image=None, lbp=None):
    ...
    # which is memory-intensive and computationally wasteful when we only need the masked pixels.
    # 📊 Impact: Bypasses a full O(W*H) array allocation and reduces redundant masking overhead.
    valid_mask = mask > 0
    valid_pixels = image[valid_mask]

    # Check if there are valid pixels before unpacking channels to avoid IndexError
