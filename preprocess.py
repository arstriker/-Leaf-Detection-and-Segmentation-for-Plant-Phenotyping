...
def extract_features(image, mask, gray_image=None, lbp=None):
    ...
    # ⚡ Bolt Optimization: Use direct boolean indexing to extract valid pixels.
    # 🎯 Why: `cv2.bitwise_and(image, image, mask=mask)` redundantly allocates a massive
    # zero-filled array for the background just to immediately throw it away during `[mask > 0]`.
    # 📊 Impact: Substantially reduces memory footprint and computational overhead by skipping full-array reallocation.
    valid_mask = mask > 0
