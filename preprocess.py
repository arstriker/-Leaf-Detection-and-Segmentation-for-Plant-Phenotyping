...
def extract_features(image, mask, gray_image=None, lbp=None):
    ...
    # ⚡ Bolt Optimization: Replace `cv2.bitwise_and` with direct NumPy boolean indexing.
    # 🎯 Why: `cv2.bitwise_and` allocates a new full-size image array which wastes memory and CPU.
    # We only need the pixels inside the mask for feature extraction. Direct indexing is faster and memory-efficient.
    bool_mask = mask > 0
    pixels = image[bool_mask]
    R = pixels[:, 0]
    G = pixels[:, 1]
    B = pixels[:, 2]
