...
def extract_features(image, mask, gray_image=None, lbp=None):
    ...
    # Performance Optimization: Instead of using cv2.bitwise_and which allocates
    # a full-size intermediate array, use direct boolean indexing to extract
    # pixel values from the original image. This saves memory and CPU cycles.
    R = image[:,:,0][mask > 0]
    G = image[:,:,1][mask > 0]
    B = image[:,:,2][mask > 0]
