...
def extract_features(image, mask, gray_image=None, lbp=None):
    ...
    # Performance optimization: Use direct boolean indexing instead of creating a full-size intermediate array with cv2.bitwise_and.
    # This reduces memory overhead and CPU cycles by avoiding an O(N) array allocation.
    mask_bool = mask > 0
    R = image[:, :, 0][mask_bool]
    G = image[:, :, 1][mask_bool]
    B = image[:, :, 2][mask_bool]
