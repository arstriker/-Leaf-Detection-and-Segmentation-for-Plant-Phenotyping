...
def extract_features(image, mask, gray_image=None, lbp=None):
    ...
    # Performance optimization: Use direct boolean indexing to extract masked pixels.
    # This avoids allocating a full-size intermediate array with cv2.bitwise_and,
    # reducing memory overhead and saving CPU cycles.
    R = image[:,:,0][mask > 0]
    G = image[:,:,1][mask > 0]
    B = image[:,:,2][mask > 0]
