...
def extract_features(image, mask, gray_image=None, lbp=None):
    ...
    # ⚡ Bolt Optimization: Use boolean indexing directly instead of cv2.bitwise_and.
    # 🎯 Why: cv2.bitwise_and allocates a full-size intermediate array with a black background,
    # and then the boolean indexing `[mask > 0]` evaluates the condition and allocates separate
    # arrays for each channel. Directly masking the original image avoids the intermediate array
    # and evaluates the condition once.
    # 📊 Impact: Significantly reduces memory allocations and speeds up extraction.
    valid_mask = mask > 0
