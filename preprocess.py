...
def extract_features(image, mask, gray_image=None, lbp=None):
    ...
    # ⚡ Bolt Optimization: Replace `cv2.bitwise_and` with direct boolean indexing.
    # 🎯 Why: `cv2.bitwise_and` allocates a full-size intermediate array with a black background, which is unnecessary
    # when we only need to extract pixels within the mask. Using direct boolean indexing `image[mask > 0]` avoids this allocation.
    # 📊 Impact: Measurably reduces memory footprint and CPU cycles by skipping the intermediate array creation.
    pixels = image[mask > 0]
    R = pixels[:, 0]
    G = pixels[:, 1]
    B = pixels[:, 2]
