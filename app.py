...
from model_inference import DiseaseClassifier, YOLOv8DiseaseClassifier, YOLOv8ObjectDetector
from database import PhenotypeDatabase

# Page config
st.set_page_config(
    page_title="Automated Leaf Detection & Phenotyping",
    page_icon="🌿",
    layout="wide"
)
...
def main():
    ...
    st.sidebar.markdown("Upload a leaf image or take a picture using your camera to begin the phenotyping pipeline.")
    ...
    if upload_option == "Upload Image":
        image_file = st.sidebar.file_uploader("Choose a plant leaf image...", type=["jpg", "jpeg", "png", "bmp"])
    else:
    ...
    if image_file is not None:
        try:
            ...
            with st.spinner("Processing image..."):
                ...
                with p_col3:
                    st.image(edges, caption="Canny Edges", use_container_width=True, channels="GRAY")
                with p_col4:
                    st.image(lbp_vis, caption="LBP (Texture)", use_container_width=True, channels="GRAY")
            ...
            with st.spinner("Segmenting and running Object Detection..."):
                ...
                with det_col2:
                    st.image(detected_leaf_img, caption="YOLOv8 Detected Bounding Boxes", use_container_width=True)
            ...
            with st.spinner("Extracting traits and classifying disease..."):
                ...
                if model_option == "YOLOv8-cls (Fast & Modern)":
                    ...
                    disease_class, confidence, probs = resnet_classifier.classify(img_np)
                ...
                with r_col1:
                    ...
                    if "error" in traits:
                        ...
                        df_traits = pd.DataFrame(
                            [{"Trait": k.replace("_", " ").title(), "Value": f"{v:.4f}" if isinstance(v, float) else str(v)} 
                            for k, v in traits.items()]
                        )
                        st.dataframe(df_traits, hide_index=True, use_container_width=True)
